...
configPlotFolder = "pyplots/configTrueSensitivity/"
utils.checkFolderExist(configPlotFolder)
...
SSOSensitivityJsonDirPath = os.path.join(utils.jsonInputs_dir, "finalSims", "EDT_True_Sensitivity")



# Load base case manually
dataSetBase = utils.getAllSimDataFromJson(os.path.join(utils.jsonInputs_dir, "finalSims", "SSO_Config_Sensitivity_Nominal.json"))
...
if doingSSOSensitivity:
    ...
    for i in range(len(typesTodo)):
        ...
        thisRunnerValuesList = utils.configTrueSensitivityRunnerValues[i]
    ...
    for i in range(len(typesTodo)):
        ...
        if plottingVsParameterValue:
            ...
            startIndex = 0
            if thisParameterType is "SecondaryTetherDiameter":
                parameterScaler = 1E3
                parameterUnits = "mm"
                # startIndex = 0

            elif "Resistivity" in thisParameterType:
                parameterScaler = 1E9
                parameterUnits = "$\Omega$nm"
            ...
            # print("HOT: ", configSensitivityRunnerValuesToPlotBare)
